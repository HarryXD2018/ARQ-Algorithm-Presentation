...
voidsender(fstream  &datafile1,fstream  &datafile2,ofstream&datafile3){
 chars,st; longi=0;  
cout<<"整个信息输入以“！”结束";
 cout<<"请输入一个待发送的字符,并按回车"; cin>>s; 
 datafile1.open("file1",ios::out|ios::in); if(!datafile1) {
  cout<<"文件打开失败！"<<endl;  exit(0); }  
 datafile1<<s;//发送信息，即将待发送的字符写入文件file1i++;
 cout<<"字符已被发送"<<endl; datafile1.close(); 
...
if(st=='N')//发送方收到接收方的出错信息NAK   {
    cout<<"上一个字符出错！"<<endl;
cout<<"现在需重新发送原字符"<<endl;    datafile1.open("file1",ios::out|ios::in);   
 if(!datafile1)     {
  cout<<"文件打开失败！"<<endl;  exit(0);     }
     datafile1<<s;i++;
 cout<<"字符已被发送"<<endl;    datafile1.close();  
    receiver(i,datafile1,datafile2,datafile3);//调用接收端进行相应操作  
 break;   }  }
   cout<<"定时器超时，现重新发送原字符。"<<endl;datafile1.open("file1",ios::out|ios::in); if(!datafile1)     {
  cout<<"文件打开失败！"<<endl;  exit(0);     }
     datafile1<<s;i++;
 cout<<"字符已被发送"<<endl;datafile1.close(); 
